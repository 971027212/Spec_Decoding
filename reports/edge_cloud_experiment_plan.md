...
| Edge-cloud Speculative, optimized | `speculative_server_accept` | Drafter on client, target HTTP service | Upload draft blocks, cloud verifies greedy acceptance, download compact acceptance result | Main optimized speculative edge-cloud method. |
| Edge-cloud Speculative, logits diagnostic | `speculative` | Drafter on client, target HTTP service | Upload draft blocks, cloud verifies, download logits/verification blocks | Diagnostic method used to show the cost of full-logits downlink. |
...
| Cloud-sim Speculative, optimized | `speculative_server_accept --simulate-network` | Drafter on client, target HTTP service | Same as optimized Edge-cloud Speculative plus simulated RTT/bandwidth | Optimized speculative method under remote-network assumptions. |
...
| `target_cloud_verify` | Cloud-side target verification for speculative draft blocks. In `speculative_server_accept`, this also includes greedy acceptance comparison on the server. |
...
| `target_tensor_materialize` | Client-side binary logits tensor reconstruction for logits-downlink speculative verification. This should be near zero in `speculative_server_accept`. |
| `acceptance_sampling` | Client-side speculative acceptance/rejection sampling. This is mainly used by the logits-downlink diagnostic mode; optimized greedy acceptance moves the comparison to the server. |
...
Run local-service cloud target and optimized speculative methods:
...
Run simulated-cloud optimized methods:
